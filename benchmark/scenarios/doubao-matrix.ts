...
import { doubaoMatrixItems } from './doubao-matrix-data';
 ...
 */
export function generateMatrixScenarioName(matrixConfig: DoubaoMatrixConfig): string {
...
export function calculateMatrixCombinations(): number {
  return doubaoMatrixItems.length;
}
...
export function printMatrixSummary(): void {
  ...
  console.log('\n=== Doubao TTS 矩阵测试计划 ===\n');
  console.log('矩阵测试列表:');
  for (const item of doubaoMatrixItems) {
 ...
 */
export async function runDoubaoMatrixScenario(options?: {
  iterations?: number;
  /** 过滤条件 */
  filter?: MatrixFilter;
  /** 任务间隔时间（毫秒），默认 1000 */
  ...
  onProgress?: (
    current: number,
    total: number,
    config: DoubaoMatrixConfig,
    result: BenchmarkResult
  ) => void;
}): Promise<BenchmarkResult[]> {
  // 动态导入以避免循环依赖
  const { runTTSTestForDoubaoMatrix } = await import('../runners/tts-runner');
  ...
  // 应用过滤条件
  const filteredItems = filterMatrixItems(doubaoMatrixItems, options?.filter);
  ...
  console.log(`\n矩阵项数量: ${filteredItems.length} (原始: ${doubaoMatrixItems.length})`);
  ...
  for (const matrixConfig of filteredItems) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const result = await runTTSTestForDoubaoMatrix(matrixConfig, text, scenarioName);
      results.push(result);
