...
import { glmMatrixItems } from './glm-matrix-data';
...
export async function runGlmMatrixScenario(options?: {
  ...
  // 动态导入以避免循环依赖
  const { runTTSTestForGlmMatrix } = await import('../runners/tts-runner');
  ...
  // 应用过滤条件
  const filteredItems = filterMatrixItems(glmMatrixItems, options?.filter);
  ...
  console.log(`\n矩阵项数量: ${filteredItems.length} (原始: ${glmMatrixItems.length})`);
  ...
  for (const matrixConfig of filteredItems) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const result = await runTTSTestForGlmMatrix(matrixConfig, text, scenarioName);
      results.push(result);
