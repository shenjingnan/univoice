...
/**
 * 运行豆包矩阵测试
 * 专门用于 Doubao TTS 矩阵测试，支持 format 和 sampleRate 参数
 */
export async function runTTSTestForDoubaoMatrix(
  matrixConfig: DoubaoMatrixConfig,
  text: TextFixture,
  scenarioName: string
): Promise<BenchmarkResult> {
  // 导入 provider 模块（导入 index 自动注册所有 provider）
...
/**
 * 运行 Minimax 矩阵测试
 * 专门用于 Minimax TTS 矩阵测试，支持 format 和 sampleRate 参数
 */
export async function runTTSTestForMinimaxMatrix(
  matrixConfig: MinimaxMatrixConfig,
  text: TextFixture,
  scenarioName: string
): Promise<BenchmarkResult> {
  // 导入 provider 模块（导入 index 自动注册所有 provider）
