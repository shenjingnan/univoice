...
function getEventTypeName(eventType: EventType): string {
  return EventType[eventType] || `invalid event type: ${eventType}`;
}

function getMsgTypeName(msgType: MsgType): string {
...
function readSessionId(msg: Message, data: Uint8Array, initialOffset: number): number {
...
function readConnectId(msg: Message, data: Uint8Array, initialOffset: number): number {
  let offset = initialOffset;
  if (msg.event === undefined) return offset;
...
function readPayload(msg: Message, data: Uint8Array, initialOffset: number): number {
  let offset = initialOffset;
  if (offset + 4 > data.length) {
