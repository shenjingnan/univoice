...
/// ```bash
/// # 使用 .env 中的凭据
/// cargo run --example tts_doubao_stream -- \
///   --output output.mp3
///
/// # 自定义文本分段
/// cargo run --example tts_doubao_stream -- \
...
struct Args {
    ...
    /// 文本分段（可指定多个，每个分段作为独立的 TaskRequest 发送）
    #[arg(
        short,
        long,
...
async fn main() {
    ...
    // 构建文本流
    let text_chunks: Vec<String> = args.text;
