 ...
 *
 * 使用方法:
 * npx tsx examples/tts/providers/qwen/cosyvoice-v1/stream-in-stream-out.ts
 */
...
// 固定使用 cosyvoice-v1 模型
const MODEL = 'cosyvoice-v1';
...
async function main() {
  ...
  console.log(`\n[${timestamp()}] === CosyVoice v1 - 流式入/流式出 ===`);
  console.log(`模型: ${MODEL}`);
  console.log(`场景: LLM 流式输出 → 实时语音合成\n`);

  // 模拟 LLM 流式输出
  const textStream = mockLLMStream(150); // 每隔 150ms 发送一个文本块
